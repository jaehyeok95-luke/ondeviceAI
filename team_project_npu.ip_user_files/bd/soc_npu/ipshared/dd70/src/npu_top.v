...
module npu_control (
    ...
    always @(*) begin
        case (cmd_layer_idx)
            3'd0: rom_shift = 4'd8;     // conv1: ci=1
            3'd1: rom_shift = 4'd13;    // conv2: ci=32
            3'd2: rom_shift = 4'd14;    // conv3: ci=64
            3'd3: rom_shift = 4'd15;    // conv4: ci=128
            3'd4: rom_shift = 4'd15;    // conv5: ci=128
            default: rom_shift = 4'd8;
        endcase
    end
            ...
            case (state)
                ...
                M_RUN: begin
                    case (mode)
                        ...
                        2'd1: begin // FC (기존 fc_step 로직 동일)
                            case (fc_step)
                                4'd0: begin fc_start<=1'b1; fc_cfg_in_len<=10'd512; fc_cfg_out_len<=9'd256; fc_cfg_relu<=1'b1;
                                            fc_w_base<=16'd0; fc_f_rd_base<=15'd0; fc_f_wr_base<=15'd512; fc_cfg_shift<=4'd14; fc_step<=4'd1; end
                                ...
                                4'd2: begin fc_start<=1'b1; fc_cfg_in_len<=10'd256; fc_cfg_out_len<=9'd19; fc_cfg_relu<=1'b0;
                                            fc_w_base<=16'd33024; fc_f_rd_base<=15'd512; fc_f_wr_base<=15'd0; fc_cfg_shift<=4'd13; fc_step<=4'd3; end
                                4'd3: if (fc_done) fc_step<=4'd4;
                                4'd4: begin fc_start<=1'b1; fc_cfg_in_len<=10'd256; fc_cfg_out_len<=9'd21; fc_cfg_relu<=1'b0;
                                            fc_w_base<=16'd34259; fc_f_rd_base<=15'd512; fc_f_wr_base<=15'd19; fc_cfg_shift<=4'd13; fc_step<=4'd5; end
                                4'd5: if (fc_done) fc_step<=4'd6;
                                4'd6: begin fc_start<=1'b1; fc_cfg_in_len<=10'd256; fc_cfg_out_len<=9'd128; fc_cfg_relu<=1'b1;
                                            fc_w_base<=16'd35624; fc_f_rd_base<=15'd512; fc_f_wr_base<=15'd40; fc_cfg_shift<=4'd13; fc_step<=4'd7; end
                                ...
                                4'd8: begin fc_start<=1'b1; fc_cfg_in_len<=10'd128; fc_cfg_out_len<=9'd28; fc_cfg_relu<=1'b0;
                                            fc_w_base<=16'd43944; fc_f_rd_base<=15'd40; fc_f_wr_base<=15'd168; fc_cfg_shift<=4'd12; fc_step<=4'd9; end
                        ...
                        2'd2: begin // Argmax
                            case (argmax_step)
                                // 초성: 현재 읽기BRAM(pong)에서
                                3'd0: begin
                                    argmax_start<=1'b1;
                                    argmax_num_classes<=5'd19;
                                    argmax_fmap_rd_base<=15'd0;
                                    argmax_step<=3'd1;
                                end
                                3'd1: if (argmax_done) begin
                                    result_cho<=argmax_result;
                                    argmax_step<=3'd2;
                                end
                        
                                // 중성: 같은 읽기BRAM(pong)에서
                                3'd2: begin
                                    argmax_start<=1'b1;
                                    argmax_num_classes<=5'd21;
                                    argmax_fmap_rd_base<=15'd19;
                                    argmax_step<=3'd3;
                                end
                                3'd3: if (argmax_done) begin
                                    result_jung<=argmax_result;
                                    // ★ BRAM 전환: head_jong 출력은 반대쪽(ping)에 있음
                                    ping_is_input<=~ping_is_input;
                                    argmax_step<=3'd4;
                                end
                        
                                // 종성: 전환된 읽기BRAM(ping)에서
                                3'd4: begin
                                    argmax_start<=1'b1;
                                    argmax_num_classes<=5'd28;
                                    argmax_fmap_rd_base<=15'd168;
                                    argmax_step<=3'd5;
                                end
                                3'd5: if (argmax_done) begin
                                    result_jong<=argmax_result;
                                    state<=M_DONE;
                                end
